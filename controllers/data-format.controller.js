...
router.get('/', async (req, res) => {
    try {
        ...
        const docs = await dataFormatModel.find(filter).select(select).sort(sort).skip(skip).limit(count).lean();
...
router.get('/:id', async (req, res) => {
    try {
        let doc = await dataFormatModel.findById(req.params.id).lean();
        if (!doc) {
