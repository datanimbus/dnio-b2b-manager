...
router.get('/', async (req, res) => {
    try {
        ...
        if (req.query.countOnly) {
            const count = await partnerModel.countDocuments(filter);
            return res.status(200).json(count);
        }
        const data = queryUtils.getPaginationData(req);
        const docs = await partnerModel.find(filter).select(data.select).sort(data.sort).skip(data.skip).limit(data.count).lean();
...
router.get('/:id', async (req, res) => {
    try {
        let doc = await partnerModel.findById(req.params.id).lean();
        if (!doc) {
...
router.post('/', async (req, res) => {
    try {
        ...
        logger.info(key);
        let doc = await partnerModel.findOne({ key });
        if (doc) {
...
router.put('/:id', async (req, res) => {
    try {
        const payload = req.body;
        let doc = await partnerModel.findById(req.params.id);
        if (!doc) {
            return res.status(404).json({
                message: 'Data Model Not Found'
            });
