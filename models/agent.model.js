...
schema.post('save', function (doc, next) {
    ...
    if (doc._req.eventId && doc._req.eventId.trim()) {
        ...
        payload.eventId = agentUtils.getAgentEventId(doc.type, doc._req.eventId);
    ...
    if (!doc._oldDoc) {
        payload.eventId = agentUtils.getAgentEventId(doc.type, 'EVENT_AGENT_APP_CREATE');
    }
    if (payload.eventId) {
        payload.priority = agentUtils.getAgentEventPriority(payload.eventId);
        ...
        client.publish(envConfig.eventsQueueName, JSON.stringify(payload));
    }
    next();
});
